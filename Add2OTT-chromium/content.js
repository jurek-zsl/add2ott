...
async function addVideoToOTT(videoUrl) {
  ...
  try {
      ...
      try {
        chrome.runtime.sendMessage({
          ...
          if (chrome.runtime.lastError) {
            // Handle the case where background script is not available
            console.warn('Background script error:', chrome.runtime.lastError);
            reject(new Error('BACKGROUND_UNAVAILABLE'));
          } else if (!response) {
            reject(new Error('BACKGROUND_UNAVAILABLE'));
          } else {
            resolve(response);
    ...
    // If background script is unavailable, show helpful message
    if (error.message === 'BACKGROUND_UNAVAILABLE' || error.message.includes('Receiving end does not exist')) {
      alert('Extension connection lost. Please reload this YouTube page and try again.');
    } else {
